...
# MAPinstance0: 2 readings in instance0
foo = as.data.frame(
  data.table::fread(
    "ukb_subset.txt.gz",
    na.strings=c("", "NA")))
foo0 = data.frame(
  ...
  event_dt = foo$p53_i0,
  trait = "MAP",
  value = (foo$p4080_i0_a0 + 2*foo$p4079_i0_a0)/3)
foo1 = data.frame(
  eid = foo$eid,
  event_dt = foo$p53_i0,
  trait = "MAP",
  value = (foo$p4080_i0_a1 + 2*foo$p4079_i0_a1)/3)
obs = rbind(foo0, foo1)
obs = obs[!is.na(obs$value), ]
...
data = obs %>%
  mutate(value=myboxcox1(value, 0.1)) %>%
  group_by(eid) %>%
  summarize(n=n(), m=mean(value), s=sd(value), .groups="drop") %>%
  filter(n>=2)
...
write.table(
  ...
  file="ukb_MAPinstance0myboxcox01nge2_QT.txt",
  sep=" ",
  quote=FALSE,
  row.names=FALSE,
  na="NA"
...
write.table(
  ...
  file="ukb_MAPinstance0myboxcox01nge2_covariates.txt",
  sep=" ",
  quote=FALSE,
  row.names=FALSE,
  na="NA"

...
a = readRDS("aucR2ROC_lipidaemiadrug_onlyaffectedexclsupplement.NGdecilecut/auc.C10AA.PC.wowinsorize.rds") %>% filter(glmnets==0.006)
a = readRDS("aucR2ROC_lipidaemiadrug_onlyaffectedexclsupplement.NGdecilecut/auc.C10AB.PC.wowinsorize.rds") %>% filter(glmnets==0.09)
a = readRDS("aucR2ROC_lipidaemiadrug_onlyaffectedexclsupplement.NGdecilecut/auc.C10AX09.PC.wowinsorize.rds") %>% filter(glmnets==0.05) 
...
a = readRDS("aucR2ROC_hypertensiondrug_onlyaffectedDBPge100SBPge160.NGdecilecut/auc.C03.PC.wowinsorize.rds") %>% filter(glmnets==0.01)
a = readRDS("aucR2ROC_hypertensiondrug_onlyaffectedDBPge100SBPge160.NGdecilecut/auc.C07.PC.wowinsorize.rds") %>% filter(glmnets==0.005)
a = readRDS("aucR2ROC_hypertensiondrug_onlyaffectedDBPge100SBPge160.NGdecilecut/auc.C08.PC.wowinsorize.rds") %>% filter(glmnets==0.004)
a = readRDS("aucR2ROC_hypertensiondrug_onlyaffectedDBPge100SBPge160.NGdecilecut/auc.C09.PC.wowinsorize.rds") %>% filter(glmnets==0.002)
...
write.table(res[, c(2:3, 1)], file="coeff.C10AX09.txt",
            row.names=F, col.names=F, sep=" ", quote=FALSE)

x = a %>% select(coef1)
x = colMeans(do.call(rbind, x[[1]]))
res = as.data.frame(x)
write.table(res, file="coeff_basal.C10AX09.txt",
...
res = read.table("coeff.C10AX09.txt", sep=" ")
...
write.table(res[, c(4, 2:3)], file="coeff.C10AX09.hg38.txt",
